...
'sub ReadBMP(In StartAddr as Long)
''********************
...
'      SPITransfer (0x51, dummy)
'      SPITransfer (StartAddr_E, dummy)   'Start address argument
'      SPITransfer (StartAddr_U, dummy)   'Start address argument
'      SPITransfer (StartAddr_H, dummy)   'Start address argument
'      SPITransfer (StartAddr, dummy)   'Start address argument
...
'   Next
'
'  fOR TRY = 1 TO 512 '512 'works when block has data
'    'wait for Data Block Transfer - first byte start block token
'    Repeat 5
'      wait 1 ms 'works with 2gb v2 card
'      SPITransfer (0xFF, SDresponse)
'      'wait 1 ms 'works with 2gb v2 card
...
'
'  For arg = 9 to 24
'    If Data(arg) = 123 Then exit For  'left opening brace
'    '#IfDef Debug_SD_ReadBMP
'      HSerPrint CHR(Data(arg))
'    '#EndIf
'  Next
...
'    HSerPrintCRLF
'  '#EndIf
'  'discard opening left brace of data block
'  arg = arg + 1
'
...
'
''#IfDef Debug_SD_ReadBMP
'  Call SD_BlockHeader(FileDataAddr)
...
'          'If arg = 512 Then Call GetMoreBlocks (StartAddr)
'
'          'second char of hex number
'          arg += 1
'          argx += 1
'
'          'start counting comma's
'              If Data(arg) = 44 Then    'check for comma
'    '#IfDef Debug_SD_ReadBMP
'                HSerSend 8              'backspace
'    '#EndIf
'                arg -= 1                'step back array
'                SDarg(argx) = Hex2Dec    'nib
'                'HSerPrint " "
'                'HSerPrint SDarg(argx)
'   ' #IfDef Debug_SD_ReadBMP
'                HSerPrint " "
'                HSerPrint CHR (nib)
'    '#EndIf
...
'                 SDarg(argx) = Hex2Dec
'                 nib = Data(arg)
'                 SDarg(argx) = (SDarg(argx) * 16) + Hex2Dec
'                'SDarg(argx) = 16 * nib + Data(arg)
'                'HSerPrint SDarg(argx)
'   ' #IfDef Debug_SD_ReadBMP
'                HSerPrint CHR (Data(arg))
'   ' #EndIf
...
'          'If arg = 512 Then Call GetMoreBlocks (StartAddr)
'
'          'discard comma after 0xXX value
'          arg += 1
'          commacount += 1
'  '#IfDef Debug_SD_ReadBMP
'          HSerPrint " "
'  '#EndIf
'          If argx = 512 Then
'              'HSerPrint SDarg(argx):HSerPrintCRLF
'            'Call SD_XferBlkData    'SD_XferBlkData_GLCD_ILI9481
'            sectorcount += 1
'            If sectorcount = 3 Then exit sub
'          End If
'
...
'  '#EndIf
'
'        If Data(arg) > 90 OR Data(arg) < 32 Then   'end of text
'          If arg = 512 Then Call SD_GetMoreData (StartAddr)
'          'If arg = 512 Then Call GetMoreBlocks (StartAddr)
'          arg += 1
'        End If
'
'        If Data(arg) = 32 Then   'space after line feed
'          If arg = 512 Then Call SD_GetMoreData (StartAddr)
'          'If arg = 512 Then Call GetMoreBlocks (StartAddr)
'           arg += 1
'        End If
...
'  SPITransfer (0xFF, SDresponse)    'Receive 16bit CRC
'  SPITransfer (0xFF, SDresponse)    'and back to idle?
'  'Call StopTransmission             'end multi block read
'  'BMPAddr = False
'end sub
...
'********************
'Dim startXpixel, SDXpixel, startYpixel, SDYpixel as Word
...
'SetAddressWindow_ILI9481 ( 0, 1, ILI9481_GLCD_WIDTH -1, ILI9481_GLCD_HEIGHT-1 )
SetAddressWindow_ILI9481 ( 0, 0, 479, 319)  'GLCDDeviceWidth -1, GLCDDeviceHEIGHT-1 )
'SetAddressWindow_ILI9481 ( 0, 0, 128, 96)  'GLCDDeviceWidth -1, GLCDDeviceHEIGHT-1 )
  ...
  For Try = 1 to 5  '255
    ...
    SPITransfer (StartAddr_H, dummy)   'Start address argument
    SPITransfer (StartAddr, dummy)   'Start address argument
    SPITransfer (0x00, dummy)         'CRC = 0x55
...
If Data(1) And Data(2) = 47 Then   'forward slash's of file header
  ...
  'HSerPrint arg:HSerPrint " "
  'discard left opening brace
'  arg += 1
'  'HSerPrint Data(arg):HSerPrint " "
'  'HSerPrintCRLF
      ...
      Do Until SD_EOF = True
        ...
        Do Until rowcount = 16
          ...
          If argx = 512 Then
            argx = 0
            sectorcount += 1
            'Call GLCDDrawBMP_ILI9481
            Call GLCDDrawBMP
              'SD_XferBlkData    'TransferBlkData
            'Call XferBlkData_GLCD_ILI9481    'TransferBlkData
            'If sectorcount = 1 Then exit sub
          End If
...
sub SD_GetMoreData (StartAddr)
  ...
  fOR TRY = 1 TO 512 'works when block has data
    'wait for Data Block Transfer - first byte start block token
    Repeat 5
      wait 1 ms 'works with 2gb v2 card
      SPITransfer (0xFF, SDresponse)
...
'Do Until SDresponse = 255
'  SPITransfer(0xFF,SDresponse)
'  count += 1
'Loop
'HSerPrint count
'HSerPrint SDresponse

  For Try = 1 to 5  '255 '25 mostly works not working with sdsc v1
...
'HSerPrint SDresponse
      ...
      SPITransfer (StartAddr_H, dummy)   'Start address argument
      SPITransfer (StartAddr, dummy)   'Start address argument
      SPITransfer (0x55, dummy)         'CRC = 0x55
